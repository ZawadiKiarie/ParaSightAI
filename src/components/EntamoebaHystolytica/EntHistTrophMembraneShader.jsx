...
const MembraneMaterialImpl = shaderMaterial(
  ...
  `,
  // Fragment Shader (Your adapted fBM logic)
  `
  uniform float iTime;
  ...
  void main() {
    // Using vUv instead of fragCoord for 3D model mapping
    vec2 uv = vUv * 2.0; 
    float shade = pattern(uv);
    vec4 col = colormap(shade);

    // Fresnel effect to make it look like a membrane boundary
    float fresnel = pow(1.0 - dot(vNormal, vec3(0,0,1)), 2.0);
    
    // Mix the pattern with transparency for a biological feel
    gl_FragColor = vec4(col.rgb, shade * 0.4 + fresnel * 0.5);
  }
  `,
);

extend({ MembraneMaterialImpl });

export const MembraneMaterial = (props) => {
  return <membraneMaterialImpl {...props} transparent />;
};
